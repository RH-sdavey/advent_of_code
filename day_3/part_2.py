...
def parse_content(in_content, index):
    ze_on = {"0": [], "1": []}
    for item in in_content:
        ze_on[item[index]].append(item)
    return ze_on


def rating(comparator):
    in_content = open('./input/input.txt').read().splitlines()
    for pos_index in range(0, 12):
        ze_on = parse_content(in_content, pos_index)
        if len(in_content := ze_on["1" if comparator(len(ze_on["1"]), len(ze_on["0"])) else "0"]) == 1:
            return int(in_content[0], 2)


print(rating(int.__ge__) * rating(int.__lt__))  # -> 4432698
